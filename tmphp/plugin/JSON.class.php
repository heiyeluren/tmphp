...
// +----------------------------------------------------------------------+
//
// $Id: JSON.php 15094 2006-07-28 22:26:55Z  $
	...
	*/
	function name_value($name, $value)
	{
		if(!is_object($value)){
			...
			return (sprintf("%s:\"\"", $this->encode(strval($name))));
		}
	}
   /** function decode
				...
				if(is_numeric($str)) { // Lookie-loo, it's a number
					...
					for($c = 0; $c < strlen($chrs); $c++) {
						...
						if(substr($chrs, $c, 2) == '\b') {
							...
							//if multibye functions are disabled process the last 2 chars, first 2 will always be zeros.
							//of the unicode string.
							//else use multi-byte decoding.
							if ($this->mbstring_enabled==0 ) {
								$utf8 .=chr(hexdec(substr($chrs, ($c+4), 2)));
								$c+=5;
							} else {
